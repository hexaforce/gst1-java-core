...
public class GSocket extends GObject {
  ...
  public GSocket bind(String address, int port) {
    ...
    if (!GIO_API.g_socket_bind(getRawPointer(), Natives.getRawPointer(boundAddress), true, reference.getPointer())) {
  ...
  public void connect(String address, int port) {
    ...
    GErrorStruct reference = new GErrorStruct();
    GErrorStruct[] errorArray = (GErrorStruct[]) reference.toArray(1);
    if (!GIO_API.g_socket_connect(getRawPointer(), Natives.getRawPointer(connectedAddress), Natives.getRawPointer(new GCancellable()), reference.getPointer())) {
      throw new GLibException(extractAndClearError(errorArray[0]));
    }
  }
